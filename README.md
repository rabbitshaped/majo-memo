# Majo Memo 🌙

This is a whimsical note-taking application inspired by Angela Yu's React course and reimagined with a cozy witchy stationery aesthetic. 
Majo Memo combines simple note-taking with magical-themed memo pads, to-do lists, reminders, and idea cards.

## Features

### Multiple note types

* Memo Pads
* To-Do Lists
* Reminder Cards
* Idea Notes

### Editing experience

* Click any note to edit it.
* Press **Enter** or click outside the note to save changes.
* Notes automatically switch between editing and reading modes.

### Visual personality

* Randomized washi tape for memo notes.
* Handwritten-style typography.
* Different backgrounds for different note types.
* Cute witch-inspired UI with soft purple tones.
* Darkmode available in toggle, for dark purple tones (work in progress).
* Local storage persists.
* Search through notes; includes empty state when nothing is found.

## Tech Stack

* React
* JavaScript
* CSS
* Lucide React Icons

## Known Issues

* Notes overflow past the footer after layout is full.
* Responsiveness is not yet added -- as it is, this app is presently desktop-only.

## Planned Features

### Organization

* Drag and drop note ordering (partially done)
* Pin/star favorite notes
* Categories and filtering (in sidebar)

### Visual Improvements

* Random washi tape rotation and mirroring
* More note backgrounds and decorations

### Functionality

* Export/import notes
* Todo progress indicators
* Ability to access and retrieve recently deleted notes

## Inspiration

This project started from Angela Yu's note-taking tutorial "Keeper" and gradually evolved into a fully customized application.

## Screenshots

<img width="500" alt="screencapture-localhost-5173-2026-06-12-15_37_41" src="https://github.com/user-attachments/assets/da8f3902-d7fe-4799-85d4-56af88d53cc9" />


---

*"Because even witches forget things."*
